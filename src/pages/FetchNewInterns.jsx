...
const FetchNewInterns = () => {
  ...
  const fetchEmails = async () => {
    ...
    try {
      ...
      console.log("Emails from API:", emailsArray);
    } catch (error) {
  ...
  const fetchLatestTime = async () => {
    try {
      const response = await api.get("/latest_uploaded_time");
  ...
  const saveEmailDataALL = async (emails) => {
    try {
      ...
      for (let i = 0; i < emails.length; i++) {
        ...
        const response = await api.post("/save_email_data", emails[i]);
  ...
  const filteredEmails = emails
    .filter((email) => {
      ...
      const searchFields = [
        ...
        email.degree,
        email.expected_role &&
          (Array.isArray(email.expected_role)
            ? email.expected_role.join(" ")
            : email.expected_role),
        email.email,
      ]
        .filter(Boolean)
            ...
            <div className="interns-controls">
                    ...
                    {uniqueRoles.slice(0, 5).map((role) => (
                      <button
                        ...
                        className={`filter-btn ${
                          activeFilter &&
                          activeFilter.key === "expected_role" &&
              ...
              <div className="sort-controls">
                ...
                <div className="sort-options">
                  ...
                  <button
                    className={`sort-btn ${
                      sortBy === "internship_period" ? "active" : ""
                    }`}
                    onClick={() => handleSort("internship_period")}
                  >
                    Duration{" "}
                    {sortBy === "internship_period" &&
                      (sortOrder === "asc" ? "↑" : "↓")}
                  </button>
                </div>
              </div>
          ...
          {isLoading ? (
            ...
            <>
              <div className="interns-count">
                ...
                {activeFilter && (
                  <span className="filtered-text"> (filtered)</span>
                )}
              </div>
                    ...
                    <div className="card-body">
                      ...
                      <div className="info-group">
                        ...
                        <div className="info-item">
                          ...
                          <div>
                            ...
                            <p>
                              {Array.isArray(email.working_mode)
                                ? email.working_mode.join(", ")
                                : email.working_mode || "N/A"}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="contact-info">
                        ...
                        <div className="contact-item">
                          ...
                          <a href={`tel:${email.contact_no}`}>
                            {email.contact_no || "N/A"}
                          </a>
                        </div>
                      </div>
                      ...
                      <div className="skills-section">
                        ...
                        <div className="skills-container">
                          {email.skills && typeof email.skills === "object" ? (
                            Object.entries(email.skills).map(
                              ([key, value], idx) => (
